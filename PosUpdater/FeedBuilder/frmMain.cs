...
namespace FeedBuilder
{
	public partial class frmMain : Form
	{
		...
		private void frmMain_Load(Object sender, EventArgs e)
		{
            ...
            _argParser = ArgumentsParser.ParseArguments(args);
		    ...
		    if (_argParser.Version != null)
		    {
		        writeVersionInfo.Checked = true;
		        textVersion.Text = _argParser.Version.ToString();
		    }
		...
		private void InitializeFormSettings()
		{
		    ...
		    writeVersionInfo.Checked = Settings.Default.WriteVersion;
		    textVersion.Text = Settings.Default.Version;
		    txtCopyInfoTo.Text = Settings.Default.CopyInfoTo;
...
// ReSharper restore AssignNullToNotNullAttribute
            ...
            Settings.Default.WriteVersion = writeVersionInfo.Checked;
            Settings.Default.Version = textVersion.Text;
		    Settings.Default.CopyInfoTo = txtCopyInfoTo.Text;
		...
		private void Build()
		{
			...
			foreach (ListViewItem thisItem in lstFiles.Items) {
				...
				if (thisItem.Checked) {
                    ...
                    if (!string.IsNullOrEmpty(fileInfoEx.FileVersion)) task.SetAttribute("version", FileVersionCondition.VersionNormalization(fileInfoEx.FileVersion));
					...
					if (chkVersion.Checked && !string.IsNullOrEmpty(fileInfoEx.FileVersion)) {
                        ...
                        cond.SetAttribute("version", FileVersionCondition.VersionNormalization(fileInfoEx.FileVersion));
						conds.AppendChild(cond);
		    ...
		    if (writeVersionInfo.Checked)
		    {
                File.WriteAllText(
                    Path.Combine(txtOutputFolder.Text.Trim(), "PosParameters.xml"), 
                    string.Format(Settings.Default.PosParametersScript, textVersion.Text));
		        if (!string.IsNullOrEmpty(txtCopyInfoTo.Text))
		        {
		            File.WriteAllText(
                        Path.Combine(txtCopyInfoTo.Text.Trim(), "!VersionUpdate.sql"),
		                string.Format(Settings.Default.SqlVersionScript, textVersion.Text, textVersion.Text));
		        }
		    ...
		    {
		        MessageBox.Show(this, @"The configuration is created.", @"Feed Builder");
		    }
		...
		private void ReadFiles()
		{
		    try
		    {
		        ...
		        if (string.IsNullOrEmpty(txtOutputFolder.Text.Trim())
		            || !Directory.Exists(txtOutputFolder.Text.Trim())) return;
