		...
		private static void Main()
		{
			...
			try {
				...
				foreach (var assemblyPath in availableAssemblies) {
                    ...
                    if (assemblyPath.Equals(Assembly.GetEntryAssembly().Location, StringComparison.InvariantCultureIgnoreCase) || assemblyPath.EndsWith(_assemblyPathEndsWith))
                    {
...
// ReSharper restore UnusedVariable
			    ...
			    try
			    {
			        dto = NauIpc.ReadDto(syncProcessName) as NauIpc.NauDto;
			    }
			    catch (Exception er)
			    {
			       Log(er);
