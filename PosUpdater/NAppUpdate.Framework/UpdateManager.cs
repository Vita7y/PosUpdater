...
namespace NAppUpdate.Framework
{
	...
	public sealed class UpdateManager
	{
	    ...
	    public static UpdateManager GetNewUpdateManager()
	    {
...
// ReSharper restore NotAccessedField.Local
	    ...
	    public EventHandler BeforeUpdateHandler;
        public EventHandler BeforeExitUpdateHandler;
        public EventHandler AfterUpdateHandler;
        public EventHandler AfterReadFeedHandler;
	    ...
	    public void CheckForUpdates(IUpdateSource source, Action<IList<IUpdateTask>> action = null)
		{
			if (IsWorking)
			...
			using (WorkScope.New(isWorking => IsWorking = isWorking))
			{
				...
				lock (UpdatesToApply)
				{
                    ...
                    if (AfterReadFeedHandler!=null)
                        AfterReadFeedHandler(tasks, EventArgs.Empty);
				    if (action != null)
                        action(tasks);
        ...
        /// <param name="updaterCanExit">true if updater can close to update himself</param>
	    /// <returns>True if successful (unless a restart was required</returns>
	    public void ApplyUpdates(bool relaunchApplication, bool updaterDoLogging, bool updaterShowConsole, bool updaterCanExit = true)
		{
			if (IsWorking)
				throw new InvalidOperationException("Another update process is already in progress");
		    ...
		    try
		    {
                if (BeforeUpdateHandler != null)
                    BeforeUpdateHandler(this, EventArgs.Empty);
...
// ReSharper restore AssignNullToNotNullAttribute
		                ...
		                if (Directory.Exists(backupParentPath) &&
		                    PermissionsCheck.HaveWritePermissionsForFolder(backupParentPath))
		                {
		                ...
		                if (revertToDefaultBackupPath)
		                {
		                    ...
		                    catch (UnauthorizedAccessException ex)
		                    {
		                        ...
		                        throw new UpdateProcessFailedException(
		                            "Could not create backup folder " + Config.BackupFolder, ex);
		                    }
		                }
		                ...
		                if (hasColdUpdates)
		                {
		                    ...
		                    var info = new ProcessStartInfo
		                    {
                                ...
                                //UserName = System.Security.Principal.WindowsIdentity.GetCurrent().Name.ToUpper().Replace("ECCO\\","")
                            };
                            ...
                            if (BeforeExitUpdateHandler != null)
                                BeforeExitUpdateHandler(this, EventArgs.Empty);
                            ...
                            var process = NauIpc.LaunchProcessAndSendDto(dto, info, Config.UpdateProcessName);
                            if (process == null)
		    ...
		    finally
		    {
		        if (AfterUpdateHandler != null)
		            AfterUpdateHandler(this, EventArgs.Empty);
		    }
		}
		...
		public void ReinstateIfRestarted()
		{
			lock (UpdatesToApply)
			{
			    NauIpc.NauDto dto = null;
			    try
			    {
			        dto = NauIpc.ReadDto(Config.UpdateProcessName) as NauIpc.NauDto;
			    }
			    catch (Exception er)
			    {
			        Logger.Log(er);
